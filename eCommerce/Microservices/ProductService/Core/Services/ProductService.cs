...
public class ProductService : IProductService
{
    ...
    public async Task<PaginatedResult<Product>> GetProducts(PaginatedDto dto)
    {
        ...
        return await fallbackPolicy.WrapAsync(circuitBreakerPolicy)
            .ExecuteAsync(async () => { return await _productRepository.GetProducts(dto.PageNumber, dto.PageSize); });
    }
    ...
    public async Task<Product> GetProductById(string id)
    {
        ...
        try
        {
            ...
            if (product == null) throw new KeyNotFoundException($"No product with id of {id}");
